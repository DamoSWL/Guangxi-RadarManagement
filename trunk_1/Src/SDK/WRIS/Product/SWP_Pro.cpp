...
#include "../../../include/WRIS/product/swp_pro.h"
...
int SWPPro::Open(string szPath)
{
	...
	if(szPath == "")
		return 1;
	////////////////////////
	memcpy( &m_SWPHeader, m_pData, sizeof(__SWP_HEADER) );
	m_pFData = (float *)(m_pData + sizeof(__SWP_HEADER));

	return 0;
}

int SWPPro::OpenBuff( char *pBuff,int iLen)
{
	...
	sprintf( cName,
		...
		ob.iProductNumber,
		ob.batch.scanmode,
		(int)m_SWPHeader.VILLenOfWin,
		m_SWPHeader.cloudType,
		(int)m_SWPHeader.LenOfWin
		);
