...
enum {
	...
	FILE_WF_ZVW_5 = 5,		//
	FILE_WF_ZVW_6 = 6,		//
	FILE_WF_ZVWH_7 = 7,		//Z、V、W、HCL
	FILE_WF_ZVWHZKRP_8 = 8,		//Z、V、W、HCL、ZDR、KDP、RHV、PDP
	FILE_WF_ZVWC_9 = 9,			//Z、V、W、CZ
	FILE_WF_ZVWHZKRPC_10 = 10	//Z、V、W、HCL、ZDR、KDP、RHV、PDP、CZ
};
namespace CDYW
{
namespace GRS
...
#pragma pack(1)
enum{
	FILE_VER_X16=0,		//16文件版本
	FILE_VER_XFL=1,		//FL文件版本
	FILE_VER_XX6R_XX6R_XQT=2,	//XX6\XX2\XQT改造文件版本
	FILE_VER_X38R=3		//X38改造文件版本
};
//! 实时数据站址参数段
...
typedef struct _REAL_CAPABILITY_SECTION // 【40】
{
	...
	unsigned short int   iPolarzation;	/*! 极化状况  【2】
											 ...
											 - 4：其它
											*/
	unsigned int	iWaveLength;			//!< 波长，以微米为单位          **  【4】
...
struct _BATCH_SCAN  // 【92】
{
	...
	char wavForm[30];		/*! spare[2 - 31], 存储体扫各层的扫描方式  【30】
								...
								- 3 
								- 4 batch 模式( RVW )
								- 8 ZPKRH 模式（ Z V W HCL ZDR KDP RHV  PDP ）
							*/
...
typedef struct _REAL_OBSERVATION_SECTION  // 【1066】
{
	...
	union  // 【120】
	{
		unsigned long id[30];				//!< ID 号，仅对体扫产品有用  【120】，径向长度
	...
	unsigned char cObsEssential[30];		/*! 观测要素            **  【30】
										 ...
										 - 12: 八要素（ R + V + W + HCL + ZDR + KDP + RHV + PDP ）
	...
	unsigned short int   iElevation[30];		/*!  各层的仰角，单位1/100度    **  【60】
									  - 做PPI时
										-# 仅在第一层填写仰角数。
									  - 做RHI时
									   -# 第一层填写方位数
	...
	union  // 【100】
	{
		...
		struct 
		{
			...
			unsigned char Fu20Height;	//-20度层高度,0.1km加权，有效范围0~200 20170107
		};		
	};	
