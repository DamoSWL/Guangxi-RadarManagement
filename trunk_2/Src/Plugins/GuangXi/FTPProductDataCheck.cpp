...
CFTPProductDataCheck::CFTPProductDataCheck(void):m_updateTime(0),m_currentDay(0),m_Lines(-1)
{
}


CFTPProductDataCheck::~CFTPProductDataCheck(void)
{
}

DWORD WINAPI CFTPProductDataCheck::fileMonitor(LPVOID para)
{
	CFTPProductDataCheck* self = static_cast<CFTPProductDataCheck*>(para);
	if(self)
	{
		UINT newTime = 0;
		auto CriticalSection = GetPlug()->getProductFTPCritical();
...
wxString CFTPProductDataCheck::getLatestFileName(UINT* newTime,const wxString& fileSpec)
{

	auto dirName = GetPlug()->getFTPProductDir();
	...
	if(dirName.IsEmpty())
	{
	 ...
	 {
	 	GetPlug()->AddLog(LOG_TYPE_ERROR,"产品FTP目录不存在，请重新选择目录");
		::wxSleep(5);
	 	return wxString();
	 }
...
void CFTPProductDataCheck::getFileContent(const char* content)
{
	if(content)
	{
...
void CFTPProductDataCheck::parseFile(UINT newTime)
{
	if(m_Contents.empty())
	{
		return;
	...
	while(istrStream >> DateStr >> TimeStr >> operationStr >> fileStr >> fileNameStr >> resultStr)
	{
		...
		if(Lines > m_Lines)
		{
			...
			if(time > 0)
			{
				if(!fileNameStr.empty() && (fileNameStr.find("P_DOR") != std::string::npos))
				{
					...
					if(writeToDataBase(singleFileInfo) && m_FTPFileAlarm.writeToDataBase(time,fileNameStr,flag))
					{
						...
						m_Lines = Lines;
						GetPlug()->setFtpProductTime(m_updateTime);
					}
				}
...
INT CFTPProductDataCheck::getDateTime(const std::string& date,const std::string time)
{
	struct tm ctime;
...
bool CFTPProductDataCheck::writeToDataBase(const std::tuple<UINT,std::string,BOOL>& baseDataInfo)
{
