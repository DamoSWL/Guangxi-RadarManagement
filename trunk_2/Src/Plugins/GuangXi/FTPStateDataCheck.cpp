...
CFTPStateDataCheck::CFTPStateDataCheck(void):m_updateTime(0),m_currentDay(0),m_Lines(-1)
{
}


CFTPStateDataCheck::~CFTPStateDataCheck(void)
{
}

DWORD WINAPI CFTPStateDataCheck::fileMonitor(LPVOID para)
{
	CFTPStateDataCheck* self = static_cast<CFTPStateDataCheck*>(para);
	if(self)
	{
		UINT newTime = 0;
		auto CriticalSection = GetPlug()->getStateFTPCritical();
...
wxString CFTPStateDataCheck::getLatestFileName(UINT* newTime,const wxString& fileSpec)
{
	auto dirName = GetPlug()->getFTPStatusDataDir();
	...
	{
		GetPlug()->AddLog(LOG_TYPE_ERROR,"状态FTP目录不存在，请重新选择目录");
		::wxSleep(5);
		return wxString();
	}
...
void CFTPStateDataCheck::getFileContent(const char* content)
{
	if(content)
	{
...
void CFTPStateDataCheck::parseFile(UINT newTime)
{
	if(m_Contents.empty())
	{
		return;
	...
	while(istrStream >> DateStr >> TimeStr >> operationStr >> fileStr >> fileNameStr >> resultStr)
	{
		...
		if(Lines > m_Lines)
		{
			...
			if(time != -1)
			{
				if(!fileNameStr.empty() && (fileNameStr.find("DWRN_SRSI") != std::string::npos))
				{
					...
					if(writeToDataBase(singleFileInfo) && m_FTPFileAlarm.writeToDataBase(time,fileNameStr,flag))
					{
						...
						m_Lines = Lines;
						GetPlug()->setFtpZhuangtaiTime(m_updateTime);
					}
				}
...
INT CFTPStateDataCheck::getDateTime(const std::string& date,const std::string time)
{
	struct tm ctime;
...
bool CFTPStateDataCheck::writeToDataBase(const std::tuple<UINT,std::string,BOOL>& baseDataInfo)
{
	...
	if(pDataBase)
	{
		try
		{
			...
			sqlSentence.Append(wxString::Format(wxT("%d, "), 3));
